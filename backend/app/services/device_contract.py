...
class DeviceContractService:
    ...
    async def sync_models_from_items(
        ...
        # preserve original casing from items（仅硬件）
        name_by_key = {
            (item.device_model_name or "").strip().lower(): (item.device_model_name or "").strip()
            for item in items
            if (item.device_model_name or "").strip()
            and (item.item_kind if item.item_kind in ("hardware", "software") else "hardware")
            == "hardware"
        }
    ...
    async def update(
        ...
        if payload.device_items is not None:
            ...
            flag_modified(entity, "device_items")
    ...
    async def summary(self) -> list[DeviceContractSummaryItem]:
        ...
        # 设备名称 -> 已上架台数（与采购汇总设备名称对齐）
        linked_rows = (await self.session.execute(linked_stmt)).all()
        linked_by_name: dict[str, int] = {}
        for dname, cnt in linked_rows:
            key = (dname or "").strip()
            if not key:
                continue
            linked_by_name[key] = linked_by_name.get(key, 0) + int(cnt)
        ...
        for (kind, mfg, name, model), bucket in sorted(
            ...
            # 按设备管理中「同设备名称」的已上架数量统计
            name_key = (name or "").strip()
            linked = linked_by_name.get(name_key, 0)
            qty = int(bucket["qty"])
            avg_price = None
