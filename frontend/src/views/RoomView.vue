...
const pageStats = computed(() => {
  ...
  const capacity = rooms.reduce((sum, r) => sum + (r.total_u || 0), 0)
  const slotCapacity = rooms.reduce((sum, r) => sum + roomCapacity(r), 0)
  const dcIds = new Set(rooms.map((r) => r.datacenter_id).filter(Boolean))
  const util = slotCapacity > 0 ? Math.round((rackCount / slotCapacity) * 100) : 0
    ...
    <section class="overview-panel">
      <div class="overview-metrics">
        ...
        <article class="metric">
          ...
          <span class="metric-hint">容量 {{ pageStats.capacity }}U · 机柜位占用 {{ pageStats.util }}%</span>
        </article>
        <article class="metric">
