      ...
      <el-form label-width="96px" size="small" @submit.prevent="handleSubmit">
        ...
        <el-form-item label="采购明细" required>
          <div class="multi-field">
            ...
            >
              <el-select v-model="item.item_kind" class="pair-kind">
                <el-option
                  v-for="opt in ITEM_KIND_OPTIONS"
                  :key="opt.value"
                  :label="opt.label"
                  :value="opt.value"
                />
...
.pair-row {
  display: grid;
  grid-template-columns: 78px 1fr 1fr 0.9fr 0.72fr 0.58fr 0.78fr 0.68fr 44px;
  gap: 6px;
  align-items: center;
}
